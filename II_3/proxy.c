...
ssize_t read_http_headers(int sock, char *buffer, size_t size)
{
    ssize_t total = 0;
    while (1) {
        ...
        ssize_t n = recv(sock, buffer + total, size - total, 0);
        if (n < 0) {
...
void *handle_client(void *arg) 
{
    ...
    ssize_t header_size = read_http_headers(client_socket, buffer, BUFFER_SIZE);
    if (header_size <= 0) {
    ...
    if (get_host(buffer, host, 0) != 0) {
        if (get_host(buffer, host, 1) != 0) {
            fprintf(stderr, "Cannot resolve host from request.\n");
            close(client_socket);
            return NULL;
        }
    }
    ...
    {
        int total_sent = 0;
        int to_send = (int)header_size;
        while (total_sent < to_send) {
            bytes_sent = send(target_socket, buffer + total_sent, to_send - total_sent, 0);
            if (bytes_sent <= 0) {
                perror("Error sending initial headers to server");
                close(client_socket);
                close(target_socket);
                return NULL;
            }
    ...
    while (!client_closed && !server_closed) 
    {
        ...
        // Если есть данные от клиента -> отправляем их серверу
        ...
        // Если есть данные от сервера -> отправляем их клиенту
...
int main() 
{
    ...
    while (1) {
        ...
        int new_socket = accept(proxy_socket, (struct sockaddr *)&client_addr, &client_addr_len);
        if (new_socket < 0) {
